...
import static org.techtown.puppydiary.kgmenu.KgPopup.puppykg;
...
public class KgTab extends AppCompatActivity {
    ...
    //double puppyjan = 0;
    double [][] kgpuppy = new double[30][12]; // 전에 10년 후에 20년, 지금이 [9][]
    public static int year_kg = 9; // 현재
...
/*
    ...
    private BarDataSet getDataSet() { //표시할 데이터 추가
        ...
        //15년간 동기화
        //kgpopup에서 받은 값 (배열) 받아와서 이 클래스 변수에 저장
        //kgpopup에서는 이 kg 값을 다른 클래스로 넘겨와주어야 하기 때문에 거기선 public으로 설정

            for (int i = 0; i < 12; i++) {
                kgpuppy[year_kg][i] = puppykg[year_kg][i];
            }

        //Log.d("kg", "kg ." + puppyjan);

        ArrayList<BarEntry> entries = new ArrayList();
        entries.add(new BarEntry(1f, (float) kgpuppy[year_kg][0]));
        entries.add(new BarEntry(2f, (float) kgpuppy[year_kg][1]));
        entries.add(new BarEntry(3f, (float) kgpuppy[year_kg][2]));
        entries.add(new BarEntry(4f, (float) kgpuppy[year_kg][3]));
        entries.add(new BarEntry(5f, (float) kgpuppy[year_kg][4]));
        entries.add(new BarEntry(6f, (float) kgpuppy[year_kg][5]));
        entries.add(new BarEntry(7f, (float) kgpuppy[year_kg][6]));
        entries.add(new BarEntry(8f, (float) kgpuppy[year_kg][7]));
        entries.add(new BarEntry(9f, (float) kgpuppy[year_kg][8]));
        entries.add(new BarEntry(10f, (float) kgpuppy[year_kg][9]));
        entries.add(new BarEntry(11f, (float) kgpuppy[year_kg][10]));
        entries.add(new BarEntry(12f, (float) kgpuppy[year_kg][11])); //아마 이게 january, 이건 확인해보면 암
